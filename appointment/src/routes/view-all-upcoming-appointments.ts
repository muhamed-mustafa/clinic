...
router.get('/api/appointment/upcoming' , requireAuth , async (req : Request , res : Response) =>
{
    ...
    const filteredAppointments = appointments.filter(appointment => { return new Date(appointment.date) > new Date() || new Date(appointment.date).getDate() === new Date().getDate() });

    res.status(200).send({ status : 200 , filteredAppointments , success : true });
});

export { router as showAllUpcomingAppointments };